...
layui.use(['laypage', 'form'], function () {
     ...
     * 置顶处理
     */
    $("#noteListContent").on('click', 'button.settingTopBtn', function () {
        var noteId = $(this).data("noteId");
        var $that = $(this);
        var url = '/admin/note/settingTop/' + noteId;
        var isTop = false;
        if ($that.find("i").hasClass("fa-circle")) {
            url = '/admin/note/cancelTop/' + noteId;
            isTop = true;
        }
        common.ajax({
            url: url,
    ...
    function buildHtml(note) {
        ...
        html += '               <i class="layui-icon">&#xe602;</i>';
        html += '           </button>';
        html += '           <button class="layui-btn layui-btn-sm margin-t-5 settingTopBtn" data-note-id="' + note.noteId + '" title="预览">';
        if (note.top) {
