...
layui.use(['laypage', 'form'], function () {
     ...
     * 预览点击
     */
    $("#articleListContent").on('click', 'button.preViewBtn', function () {
        var articleId = $(this).data("articleId");
        window.open("/admin/article/preview/" + articleId);
    });
     ...
     * 审核点击
     */
    $("#articleListContent").on('click', 'button.auditBtn', function () {
        var articleId = $(this).data("articleId");
        common.ajax({
    ...
    $("body").on('click', 'form.audit-form a.submitForm', function () {
        ...
        common.ajax({
            url: "/admin/article/audit/" + $thatForm.data("bizId"),
            type: "POST",
            data: requestData,
            traditional: true,
            success: function (res) {
                if (res.result === 'success') {
                    $("#articleListContent").find("tr#article-" + $thatForm.data("bizId")).remove();
    ...
    function buildHtml(article) {
        var html = '';
        html += '<tr id="article-' + article.articleId + '">';
        ...
        html += '           <button class="layui-btn layui-btn-sm margin-t-5 auditBtn" data-article-id="' + article.articleId + '" title="审核">';
        html += '               <i class="layui-icon">&#xe642;</i>';
        html += '           </button>';
        html += '           <button class="layui-btn layui-btn-sm margin-t-5 removeBtn" data-article-id="' + article.articleId + '" title="删除">';
        html += '               <i class="layui-icon">&#xe640;</i>';
        html += '           </button>';
